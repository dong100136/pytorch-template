...
class TaskDataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        spt_y_in = [torch.tensor(self.inp2img(sample['output'])).float().unsqueeze(0) for sample in task['train']]
        ...
        qry_y_in = [torch.tensor(self.inp2img(sample['output'])).float().unsqueeze(0) for sample in task['test']]
        qry_y = [torch.tensor(np.array(sample['output'])).long().unsqueeze(0) for sample in task['test']]

        return spt_x, spt_y_in, spt_y, qry_x, qry_y_in, qry_y
