...
from CALearner import CALearner
...
hidden_cells = 256
model_config = [
    ('conv2d', [hidden_cells, num_states, 3, 3, 1, 1]),
    ('relu', [True]),
    ('conv2d', [num_states, hidden_cells, 1, 1, 1, 0]),
]
...
def main():
    model = CALearner(model_config)
    model = load_model(model)
    maml = Meta(exp_config, model, loop_steps=exp_config['loop_steps']).to(device)
